...
TestCases = {
    "generic": (valid_smiles_sentences, valid_smiles_prefixes, invalid_smiles_sentences),
    "isocyanates": (valid_isocyanite_sentences, valid_smiles_sentences, invalid_smiles_sentences),
    "acrylates": (valid_acrylate_sentences, valid_smiles_sentences, invalid_smiles_sentences),
    "chain_extenders": (valid_chain_extender_sentences, valid_smiles_sentences, invalid_smiles_sentences),
}
...
class Test_parsing_smiles_object(TestCase):
    def setUp(self):
        ...
        for grammar_name in TestCases:
            ...
            self.recognizers[grammar_name] = StringRecognizer(parsed_grammar.grammar_encoding, start_rule_id)
    ...
    def test_valid_sentence(self):
        for grammar_name, recognizer in self.recognizers.items():
            ...
            for molecule_test_case in valid_full:
                fail_msg = f"{grammar_name.capitalize()}:" +  f"Failed on {molecule_test_case.name}, {molecule_test_case.molecule}"
