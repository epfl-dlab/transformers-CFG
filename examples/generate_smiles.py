...
def parse_args():
    ...
    parser.add_argument("--smiles-type", type=str, choices=["generic", "isocyanates", "acrylates", "chain_extenders"], default="generic", help="Type of SMILES to generate")
...
if __name__ == "__main__":
    ...
    for generation, gen_type in zip(generations, ['Unconstrained:', 'Constrained:']):
        print(gen_type)
        print(generation)
        print(f'The generation matches the grammar: {string_grammar._accept_string(generation[len(prefix1):])}')
        print(f'The generated prefix matches the grammar: {string_grammar._accept_prefix(generation[len(prefix1):])}')
