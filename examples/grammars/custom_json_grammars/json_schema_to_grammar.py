...
def parse_args():
    ...
    parser.add_argument(
        "-i",
        "--input-schema-path",
        help="Path or link (if fetching is allowed) to a JSON schema file",
        type=str,
        required=True,
    )
    parser.add_argument(
        "-o",
        "--output-grammar-path",
        help="Path to the corresponding EBNF grammar file",
        type=str,
        required=True,
    )
    parser.add_argument(
...
def main():
    ...
    if args.input_schema_path.startswith("https://"):
        url = args.input_schema_path
        schema = requests.get(url).json()
    else:
        url = f"file://{args.input_schema_path}"
        with open(args.input_schema_path) as f:
            schema = json.load(f)
    ...
    with open(args.output_grammar_path, "w") as f:
        f.write(converter.format_grammar())
